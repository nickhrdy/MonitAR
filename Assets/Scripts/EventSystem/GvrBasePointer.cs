...
public abstract class GvrBasePointer : MonoBehaviour, IGvrControllerInputDeviceReceiver
{
    ...
    void Update()
    {
        ...
        {
            float tempx = Input.GetAxis("Mouse X") - mouse_X;
            float tempy = Input.GetAxis("Mouse Y") - mouse_Y;
            float vecX = Mathf.Sqrt(Mathf.Pow(tempx, 2) / 2);
            float vecY = Mathf.Sqrt(Mathf.Pow(tempy, 2) / 2);
            Vector3 vec = new Vector3(transform.position.x + vecX, transform.position.y + vecY, transform.position.z + vecX + vecY);
    ...
    */
    ...
    private void UpdateTriggerState()
    {
        if (lastUpdateFrame != Time.frameCount)
        {
            ...
            {
                buttonDown |= GvrControllerButton.System;
            }

            if (Input.GetMouseButtonUp(3))
            {
                buttonUp |= GvrControllerButton.System;
            }
